/**
 * Seed script — populates the product documents in Sanity
 * with all the current live static products.
 *
 * Run with:
 *   SANITY_AUTH_TOKEN=your_token node scripts/seed-products.mjs
 */
...
const staticProducts = [
  ...
  },
  {
    ...
    weight: "On request",
    finish: "Hand Painted with Protective PU Coat",
    deck: "Petals of Reverie carries a celebratory softness. Its sunflower-like center and repeating teardrop petals give the piece a joyful cadence, while the layered construction preserves depth and sophistication. It is especially effective in interiors that want color without losing refinement.",
    maker: "Wallantq Private Atelier",
    leadTime: "Ready to ship · 7 days",
    theme: "Sacred Bloom",
...
async function seed() {
  ...
  try {
    for (const p of staticProducts) {
      // Set featured/newArrival properties based on hardcoded homepage defaults
      const isFeatured = ["wanxmf05", "wmnxmf22", "wmnxmf27_sq", "wmnxmf39", "winxmf01"].includes(p.slug)
      const isNewArrival = ["wmnxmf27", "wmnxmf16", "wmnxmf02", "wmnxmf05"].includes(p.slug)

      const doc = {
        _id: `product_${p.sku.toLowerCase()}`,
        _type: 'product',
        title: p.title.trim(),
        ...
        mood: p.mood || '',
        tags: p.tags || [],
        mediaSource: 'hostinger',
        hasVideo: p.hasVideo,
        featured: isFeatured,
        newArrival: isNewArrival,
        provenance: p.provenance.map((item, i) => ({
          _key: `prov_${i}`,
          title: item.title,
          body: item.body,
        })),
      ...
      console.log(`   - Uploading: ${doc.title} (${doc.sku})`)
